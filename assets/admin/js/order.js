(function ($) {
    ...
    function dpdRoResponse(modal, message, type) {
        var response = `
            <div class="d-message d-` + type + `">(function ($) {
                ...
                success: function (response) {
                    ...
                    if (shipments.length > 0 && shipments[0].service?.additionalServices?.cod?.amount !== undefined) {
                        ...
                        shipments.forEach((shipment, index) => {
                            const priceString = `Shipment #${index + 1}: amount ${shipment.service.additionalServices.cod.amount} ${shipment.service.additionalServices.cod.currencyCode}`;
                            alert(priceString);
