	...
	class Puc_v4p11_Plugin_Package extends Puc_v4p11_InstalledPackage {
		...
		public function isMuPlugin() {
			...
			if ( $cachedResult === null ) {
				if ( !defined('WPMU_PLUGIN_DIR_DPDRO') || !is_string(WPMU_PLUGIN_DIR_DPDRO) ) {
					$cachedResult = false;
					return $cachedResult;
				}

				//Convert both paths to the canonical form before comparison.
				$muPluginDir = realpath(WPMU_PLUGIN_DIR_DPDRO);
				$pluginPath  = realpath($this->pluginAbsolutePath);
				//If realpath() fails, just normalize the syntax instead.
				if (($muPluginDir === false) || ($pluginPath === false)) {
					$muPluginDir = Puc_v4p11_Factory::normalizePath(WPMU_PLUGIN_DIR_DPDRO);
